<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vtp_manage_portal_contact" style="margin-top: 5px !important;">
      <thead>
          <tr>
          
          <?php include_partial('adManageContact/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $ad_contact): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $ad_contact->getId() ?>}" rel="<?php echo $ad_contact->getId() ?>">

                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('adManageContact/list_td_tabular', array('ad_contact' => $ad_contact, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('adManageContact/list_td_actions', array('ad_contact' => $ad_contact, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="6">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('adManageContact/pagination', array('pager' => $pager)) ?></div>
