<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_ad_manage_album" style="margin-top: 5px !important;">
      <thead>
          <tr>
          ...
          <?php include_partial('adAlbum/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $ad_album): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $ad_album->getId() ?>}" rel="<?php echo $ad_album->getId() ?>">

                            <?php include_partial('adAlbum/list_td_batch_actions', array('ad_album' => $ad_album, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('adAlbum/list_td_tabular', array('ad_album' => $ad_album, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('adAlbum/list_td_actions', array('ad_album' => $ad_album, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="6">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('adAlbum/pagination', array('pager' => $pager)) ?></div>
