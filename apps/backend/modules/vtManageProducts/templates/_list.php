<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vt_manage_products" style="margin-top: 5px !important;">
      <thead>
          <tr>
          ...
          <?php include_partial('vtManageProducts/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $vtp_products): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $vtp_products->getId() ?>}" rel="<?php echo $vtp_products->getId() ?>">

                            <?php include_partial('vtManageProducts/list_td_batch_actions', array('vtp_products' => $vtp_products, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('vtManageProducts/list_td_tabular', array('vtp_products' => $vtp_products, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('vtManageProducts/list_td_actions', array('vtp_products' => $vtp_products, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="8">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('vtManageProducts/pagination', array('pager' => $pager)) ?></div>
