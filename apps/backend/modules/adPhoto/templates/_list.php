<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_ad_manage_photo" style="margin-top: 5px !important;">
      <thead>
          <tr>
          ...
          <?php include_partial('adPhoto/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $ad_photo): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $ad_photo->getId() ?>}" rel="<?php echo $ad_photo->getId() ?>">

                            <?php include_partial('adPhoto/list_td_batch_actions', array('ad_photo' => $ad_photo, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('adPhoto/list_td_tabular', array('ad_photo' => $ad_photo, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('adPhoto/list_td_actions', array('ad_photo' => $ad_photo, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
            ...
            <div style="position: relative; width: auto; float:right">
                <?php 
                    ...
                    if($request->hasParameter('default_album_id')){
                        include_partial('adPhoto/pagination', array('pager' => $pager,'default_album_id'=>$request->getParameter('default_album_id'))); 
                    }
                    else{
                        include_partial('adPhoto/pagination', array('pager' => $pager)); 
                    }
