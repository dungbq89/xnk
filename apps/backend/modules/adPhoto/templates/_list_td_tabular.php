  ...
  <td class="sf_admin_text sf_admin_list_td_name" field="name">
      <?php 
        if($request->hasParameter('default_album_id')){
            echo  link_to(VtHelper::truncate($ad_photo->getName(), 50, '...', true),'ad_photo_edit',array('id'=>$ad_photo->getId(),'default_album_id'=>$request->getParameter('default_album_id')));
        }
        else {
            echo  link_to(VtHelper::truncate($ad_photo->getName(), 50, '...', true),'ad_photo_edit',$ad_photo);
        }
      ?>
  </td>      
