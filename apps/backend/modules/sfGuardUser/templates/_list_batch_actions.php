...
<!---->
<!--	  --><?php //endif;?>
        <button onclick="$(this).addClass('clicked'); return confirm('<?php echo __('Bạn có chắc muốn khóa không?') ?>')" type="submit" class="btn btn-success" name="batch_action"
                value="batchDeactive"><?php echo __('Khóa', array(), 'messages') ?></button>
        <button onclick="$(this).addClass('clicked'); return confirm('<?php echo __('Bạn có chắc muốn mở khóa không?') ?>')" type="submit" class="btn btn-success" name="batch_action"
