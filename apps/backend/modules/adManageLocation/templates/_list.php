<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vt_manage_advertise_location" style="margin-top: 5px !important;">
      <thead>
          <tr>
          ...
          <?php include_partial('adManageLocation/list_th_tabular', array('sort' => $sort)) ?>

                        <th id="sf_admin_list_th_actions" style="width: 200px;text-align: center"><?php echo __('Actions', array(), 'tmcTwitterBootstrapPlugin') ?></th>
                    </tr>
      </thead>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $ad_advertise_location): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $ad_advertise_location->getId() ?>}" rel="<?php echo $ad_advertise_location->getId() ?>">

                            <?php include_partial('adManageLocation/list_td_batch_actions', array('ad_advertise_location' => $ad_advertise_location, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('adManageLocation/list_td_tabular', array('ad_advertise_location' => $ad_advertise_location, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('adManageLocation/list_td_actions', array('ad_advertise_location' => $ad_advertise_location, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="7">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('adManageLocation/pagination', array('pager' => $pager)) ?></div>
