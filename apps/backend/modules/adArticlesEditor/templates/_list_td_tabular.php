  ...
  <td class="sf_admin_boolean sf_admin_list_td_is_hot" field="is_hot" style="text-align: center"><?php echo get_partial('adArticlesEditor/list_field_boolean', array('value' =>  VtHelper::truncate($ad_article->getIsHot(), 50, '...', true) )) ?></td>
  ...
  <td class="sf_admin_text sf_admin_list_td_is_active" field="is_active" style="text-align: center">
      <?php  
            if($ad_article->getIsActive()=='0'){
                echo __('Đợi duyệt');
            }elseif($ad_article->getIsActive()=='1'){
                echo __('Đã duyệt');
            }elseif($ad_article->getIsActive()=='2'){
                echo __('Đã đăng');
            }else{echo __('Bản nháp');}
      ?>
  </td>    
  <td class="sf_admin_foreignkey sf_admin_list_td_created_by" field="created_by"><?php echo  VtHelper::truncate($ad_article->getCreatedBy(), 50, '...', true)  ?></td>
  <td class="sf_admin_foreignkey sf_admin_list_td_created_at" field="updated_at" style="text-align: center; white-space: nowrap">
      <?php echo  date('d-m-Y H:i:s',  strtotime($ad_article->created_at))  ?></td>