<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_vt_manage_html" style="margin-top: 5px !important;">
      <thead>
          <tr>
          ...
          <?php include_partial('adManageHtml/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $ad_html): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $ad_html->getId() ?>}" rel="<?php echo $ad_html->getId() ?>">

                            <?php include_partial('adManageHtml/list_td_batch_actions', array('ad_html' => $ad_html, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('adManageHtml/list_td_tabular', array('ad_html' => $ad_html, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('adManageHtml/list_td_actions', array('ad_html' => $ad_html, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="7">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('adManageHtml/pagination', array('pager' => $pager)) ?></div>
