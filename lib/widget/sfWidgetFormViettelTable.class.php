  ...
  private function getRecords($value) {
    ...
    if ($relationType == "FIELD"){
        if(is_null($value)==false && $value!=""){
          $results = Doctrine_Core::getTable($model)->$tableMethod(explode(',', $value));
        }else{
          return array();
        }
    }
      ...
      if(is_null($value)==false && count($value)>0){
        ...
        if(is_null($tableMethodInput)){
          ...
          if(is_null($value2)==false){
            $value2 = $this->getOption('second_param_for_query');
            $results = Doctrine_Core::getTable($model)->$tableMethod($value,$value2);
          }else{
            $results = Doctrine_Core::getTable($model)->$tableMethod($value); 
          }
        }else if(is_null($tableMethodInput)==false){
          $results = Doctrine_Core::getTable($model)->$tableMethod($tableMethodInput);
        }
      }else{
        return array();
      }
